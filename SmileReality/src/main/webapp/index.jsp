<!DOCTYPE html>
<html>
<head>
<title>Smile Reality</title>
<link rel="stylesheet" href="css/style.css" type="text/css" />
<link rel="stylesheet" href="css/responsive.css" type="text/css" />
<link rel="stylesheet" href="css/flexslider.css" type="text/css" media="screen" />
<script src="http://ajax.googleapis.com/ajax/libs/jquery/1/jquery.min.js"></script>

<script>

function fixedscheme()
{
	$(".home").hide();
	$(".deposit").show();
	$(".reccuring").hide();
	$(".certificate").hide();
	$(".contactus").hide();
}
	function recurringscheme()
	{
		
		$(".home").hide();
		$(".deposit").hide();
		$(".reccuring").show();
		$(".certificate").hide();
		$(".contactus").hide();
	}
	function companycertificate()
	{
		$(".home").hide();
		$(".deposit").hide();
		$(".reccuring").hide();
		$(".certificate").show();
		$(".contactus").hide();
	}
	function contactdetails()
	{
		$(".contactus").show();
		$(".home").hide();
		$(".deposit").hide();
		$(".reccuring").hide();
		$(".certificate").hide();
	}
	function home()
	{
		
		$(".home").show();
		$(".contactus").hide();
		$(".deposit").hide();
		$(".reccuring").hide();
		$(".certificate").hide();
	}
</script>

</head>

<body>
<div class="container">
   <header class="headerbox">
    <div class="logo"><img src="images/logo2.jpg" width="643" height="91"></div>
    <div class="phoneno">
    <a href="#"><img src="images/icon-fb.jpg"></a>
<a href="#"><img src="images/icon-tweet.jpg"></a>    
    </div>
    <div class="updated">YC is one time exercise while dealing in securities markets - once KYC is done through a SEBI registered intermediary (broker, DP, Mutual Fund etc.), you need not undergo the same process again when</div>
    <div class="nav-box">
      <div class="nav">
        <ul>
          <li><a onclick="home()" href="#">home</a></li>
         
          <li><a onclick="companycertificate()" href="#">company</a></li>

          <li><a href="#">plan</a></li>
          <li><a href="#" onclick="contactdetails()">Contact us</a> </li>
        </ul>
        <div class="regi"><img src="images/icon-register.png"><span>Register Now</span></div>
      </div>
      <div class="nav-shade"></div>
    </div>
  </header>
  <aside class="banner-main">
    <nav class="leftnav">
      <ul>
       
        <li><a href="#" onclick="fixedscheme()"><span><img src="images/icon-deposite.png"></span>Fixed Schemes</a></li>
        <li><a href="#" onclick="recurringscheme()" ><span><img src="images/icon-personal-loan.png"></span>Rucurring Schemes</a></li>
        <li><a href="#"><span><img src="images/icon-saving.png"></span>Saving Account</a></li>
      </ul>
    </nav>
    <aside class="banner-slider">
      <div id="sliderbox">
        <div class="flexslider">
          <ul class="slides">
            <li> <img src="images/slide1.png"/> </li>
            <li> <img src="images/slide2.png"/> </li>
            <li> <img src="images/slide3.png"/> </li>
          </ul>
        </div>
        
        <!-- jQuery --> 
        
        <!-- FlexSlider --> 
        <script defer src="js/jquery.flexslider.js"></script> 
        <script type="text/javascript">
    $(function(){
      SyntaxHighlighter.all();
    });
    $(window).load(function(){
      $('.flexslider').flexslider({
        animation: "slide",
        start: function(slider){
          $('body').removeClass('loading');
        }
      });
    });
  </script> 
      </div>
    </aside>
  </aside>
  <article class="welcome-loginmain">
    <aside class="left-box">
      <div class="welcm home">
        <h1>Smile Reality Producer Company Limited</h1>
        <p>
        <b class="color"> Object -</b> To carry on the business of cultivation, manufacturers, producers, collectors, growing in green house, maintain, cold storage, purifiers, importers,  exporters, traders, distributors and dealers in floriculture, plantations, mushroom cultivation & processing organic manure, culture, vegetable, fruits agricultural, medicinal, spices crops, poultry, apiculture, sericulture, their processed products and other allied products, seeds and horticultural products and manufacturing, canning tinning, dispose of and deal in the derivatives of said products, including contract farming and training.
	To undertake Organic production programmer processing & Certification, Exports & trade business ( as per the Nation and International norms & standard)ll the  of all the agricultural, Horticultural Vegetables crops and Dairy products their   allied forms, by the members & non members producer of the company. It will include import & hire of goods and services in all forms , for the benefit of the members.
         
        </p>
      </div>
      
      <div class="welcm certificate" style="display:none">
       <h1>Smile Reality Certificate</h1>
        <img src="images/certificate.jpg" />
      </div>
       <div class="welcm contactus" style="display:none">
       <h1>Contact Us</h1>
    <p> Head Office- 2nd Floor, Opp Aroma Hetel , Dabwali Road, Sirsa- 125055  <br/>
      Phone No- 082219-84669 </p>
      </div>
      
       <div class="welcm deposit" style="display:none">
        <h1>Fixed Deposit</h1>
        <p style="margin-top:10px">
      Fixed deposit (FD) is a financial instrument where a sum of money given to a bank, financial institution or company whereby the receiving entity pays interest at a specified percentage for the time duration of the deposit. The rate of interest paid for fixed deposit vary according to amount, period and from bank to bank. At the end of the time period of the deposit the amount that is originally given is returned to the investor.
         <br/>
         Duration of Fixed Deposit
Fixed deposit can be opened for a minimum period of 1 Year to maximum of 10 years.
Eligibility for Fixed Deposit
All Resident individuals are eligible to open a fixed deposit account
Plan for FD in Smile Reality Producer company
         <br/>
       <table class="fixeddeposit" border="1" cellspacing="0" cellpadding="0">
    <tbody>
        <tr>
            <td width="117" valign="top">
                <p>
                    Sr No.
                </p>
            </td>
            <td width="117" valign="top">
            </td>
            <td width="117" valign="top">
                <p>
                    1 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    2 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    3 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    4 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    5 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    6 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    8 Year
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    10 Year
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
            </td>
            <td width="117" valign="top">
                <p>
                    Deposit
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity<strong></strong>
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    Maturity
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    1
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    10000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    11038
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    12184
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    13449
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    15435
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    18061
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    20328
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    27838
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    37724
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    2
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    20000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    22076
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    24898
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    26898
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    30870
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    36122
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    40656
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    55656
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    75448
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    3
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    30000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    33114
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    36552
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    40347
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    46305
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    54183
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    60984
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    83484
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    113172
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    4
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    50000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    55190
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    60920
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    67245
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    77175
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    90305
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    101640
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    139140
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    188621
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    5
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    100000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    110381
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    121840
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    134489
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    154350
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    180611
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    203279
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    278280
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    377243
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    6
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    200000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    220762
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    243680
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    268978
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    308702
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    361222
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    406559
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    556560
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    754485
                </p>
            </td>
        </tr>
        <tr>
            <td width="117" valign="top">
                <p>
                    7
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    400000
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    441525
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    487360
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    537955
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    617404
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    722444
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    813118
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    1113120
                </p>
            </td>
            <td width="117" valign="top">
                <p>
                    1508971
                </p>
            </td>
        </tr>
    </tbody>
</table>
         
        </p>
      </div>
        <div class="welcm reccuring" style="display:none">
        <h1>Recurring Deposit </h1>
        <p style="margin-top:10px">
      <b> Recurring Deposit </b> is a special kind of Term Deposit offered by banks in India which help people with regular incomes to deposit a fixed amount every month into their<b> Recurring Deposit</b> account and earn interest at the rate applicable to Fixed Deposits.
         <br/>
      <div class="table-wraper">
       <table class="recurringdeposit" id="recurringtable" border="1" cellpadding="0" cellspacing="0">
       
    <tbody>
        <tr>
            <td valign="top">
                <p>
                    Sr No
                </p>
            </td>
            <td valign="top" >
            </td>
            <td valign="top" >
            </td>
            <td valign="top" >
            </td>
            <td valign="top" >
            </td>
            <td colspan="2" valign="top">
                <p>
                    1 Year
                </p>
            </td>
            <td colspan="2" valign="top" >
                <p>
                    2 Year
                </p>
            </td>
            <td colspan="2" valign="top" >
                <p>
                    3 Year
                </p>
            </td>
            <td colspan="2" valign="top" >
                <p>
                    4 Year
                </p>
            </td>
            <td colspan="2" valign="top" >
                <p>
                    5 Year
                </p>
            </td>
            <td colspan="2" valign="top" >
                <p>
                    6 Year
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
            </td>
            <td valign="top">
                <p>
                    MLY
                </p>
            </td>
            <td valign="top" >
                <p>
                    QLY
                </p>
            </td>
            <td valign="top" >
                <p>
                    HLY
                </p>
            </td>
            <td valign="top">
                <p>
                    YLY
                </p>
            </td>
            <td valign="top" >
                <p>
                    Deposit
                </p>
            </td>
            <td valign="top" >
                <p>
                    Maturity
                </p>
            </td>
            <td valign="top" >
                <p>
                    Deposit
                </p>
            </td>
            <td valign="top" >
                <p>
                    Maturity
                </p>
            </td>
            <td valign="top">
                <p>
                    Deposit
                </p>
            </td>
            <td valign="top">
                <p>
                    Maturity
                </p>
            </td>
            <td valign="top" >
                <p>
                    Deposit
                </p>
            </td>
            <td valign="top" >
                <p>
                    Maturity
                </p>
            </td>
            <td valign="top">
                <p>
                    Deposit
                </p>
            </td>
            <td valign="top" >
                <p>
                    Maturity
                </p>
            </td>
            <td valign="top" >
                <p>
                    Deposit
                </p>
            </td>
            <td valign="top" >
                <p>
                    Maturity
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
                <p>
                    1
                </p>
            </td>
            <td valign="top" >
                <p>
                    200
                </p>
            </td>
            <td valign="top" >
                <p>
                    597
                </p>
            </td>
            <td valign="top" >
                <p>
                    1188
                </p>
            </td>
            <td valign="top" >
                <p>
                    2364
                </p>
            </td>
            <td valign="top" >
                <p>
                    2400
                </p>
            </td>
            <td valign="top" >
                <p>
                    2533
                </p>
            </td>
            <td valign="top" >
                <p>
                    4800
                </p>
            </td>
            <td valign="top" >
                <p>
                    5329
                </p>
            </td>
            <td valign="top" >
                <p>
                    7200
                </p>
            </td>
            <td valign="top" >
                <p>
                    8415
                </p>
            </td>
            <td valign="top" >
                <p>
                    9600
                </p>
            </td>
            <td valign="top">
                <p>
                    12076
                </p>
            </td>
            <td valign="top" >
                <p>
                    12000
                </p>
            </td>
            <td valign="top" >
                <p>
                    16445
                </p>
            </td>
            <td valign="top" >
                <p>
                    14400
                </p>
            </td>
            <td valign="top">
                <p>
                    21069
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top">
                <p>
                    2
                </p>
            </td>
            <td valign="top" >
                <p>
                    300
                </p>
            </td>
            <td valign="top">
                <p>
                    895
                </p>
            </td>
            <td valign="top" >
                <p>
                    1782
                </p>
            </td>
            <td valign="top" >
                <p>
                    3548
                </p>
            </td>
            <td valign="top">
                <p>
                    3600
                </p>
            </td>
            <td valign="top">
                <p>
                    3800
                </p>
            </td>
            <td valign="top" >
                <p>
                    7200
                </p>
            </td>
            <td valign="top" >
                <p>
                    7994
                </p>
            </td>
            <td valign="top" >
                <p>
                    10800
                </p>
            </td>
            <td valign="top" >
                <p>
                    12626
                </p>
            </td>
            <td valign="top" >
                <p>
                    14400
                </p>
            </td>
            <td valign="top">
                <p>
                    18114
                </p>
            </td>
            <td valign="top" >
                <p>
                    18000
                </p>
            </td>
            <td valign="top" >
                <p>
                    24667
                </p>
            </td>
            <td valign="top" >
                <p>
                    21600
                </p>
            </td>
            <td valign="top" >
                <p>
                    31603
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
                <p>
                    3
                </p>
            </td>
            <td valign="top" >
                <p>
                    500
                </p>
            </td>
            <td valign="top" >
                <p>
                    1492
                </p>
            </td>
            <td valign="top" >
                <p>
                    2970
                </p>
            </td>
            <td valign="top" >
                <p>
                    5910
                </p>
            </td>
            <td valign="top">
                <p>
                    6000
                </p>
            </td>
            <td valign="top" >
                <p>
                    6333
                </p>
            </td>
            <td valign="top" >
                <p>
                    12000
                </p>
            </td>
            <td valign="top" >
                <p>
                    13323
                </p>
            </td>
            <td valign="top" >
                <p>
                    18000
                </p>
            </td>
            <td valign="top" >
                <p>
                    21038
                </p>
            </td>
            <td valign="top" >
                <p>
                    24000
                </p>
            </td>
            <td valign="top" >
                <p>
                    30189
                </p>
            </td>
            <td valign="top" >
                <p>
                    30000
                </p>
            </td>
            <td valign="top" >
                <p>
                    41112
                </p>
            </td>
            <td valign="top">
                <p>
                    36000
                </p>
            </td>
            <td valign="top" >
                <p>
                    52672
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
                <p>
                    4
                </p>
            </td>
            <td valign="top" >
                <p>
                    1000
                </p>
            </td>
            <td valign="top" >
                <p>
                    2985
                </p>
            </td>
            <td valign="top" >
                <p>
                    5940
                </p>
            </td>
            <td valign="top" >
                <p>
                    11820
                </p>
            </td>
            <td valign="top" >
                <p>
                    12000
                </p>
            </td>
            <td valign="top" >
                <p>
                    12665
                </p>
            </td>
            <td valign="top" >
                <p>
                    24000
                </p>
            </td>
            <td valign="top">
                <p>
                    26645
                </p>
            </td>
            <td valign="top" >
                <p>
                    36000
                </p>
            </td>
            <td valign="top">
                <p>
                    42076
                </p>
            </td>
            <td valign="top" >
                <p>
                    48000
                </p>
            </td>
            <td valign="top" >
                <p>
                    60379
                </p>
            </td>
            <td valign="top">
                <p>
                    60000
                </p>
            </td>
            <td valign="top" >
                <p>
                    82223
                </p>
            </td>
            <td valign="top" >
                <p>
                    72000
                </p>
            </td>
            <td valign="top" >
                <p>
                    105345
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
                <p>
                    5
                </p>
            </td>
            <td valign="top" >
                <p>
                    2000
                </p>
            </td>
            <td valign="top" >
                <p>
                    5970
                </p>
            </td>
            <td valign="top" >
                <p>
                    11880
                </p>
            </td>
            <td valign="top">
                <p>
                    23640
                </p>
            </td>
            <td valign="top">
                <p>
                    24000
                </p>
            </td>
            <td valign="top">
                <p>
                    25330
                </p>
            </td>
            <td valign="top">
                <p>
                    48000
                </p>
            </td>
            <td valign="top">
                <p>
                    53290
                </p>
            </td>
            <td valign="top" >
                <p>
                    72000
                </p>
            </td>
            <td valign="top" >
                <p>
                    84153
                </p>
            </td>
            <td valign="top" >
                <p>
                    96000
                </p>
            </td>
            <td valign="top" >
                <p>
                    120758
                </p>
            </td>
            <td valign="top" >
                <p>
                    120000
                </p>
            </td>
            <td valign="top" >
                <p>
                    164447
                </p>
            </td>
            <td valign="top">
                <p>
                    144000
                </p>
            </td>
            <td valign="top" >
                <p>
                    210690
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
                <p>
                    6
                </p>
            </td>
            <td valign="top" >
                <p>
                    5000
                </p>
            </td>
            <td valign="top" >
                <p>
                    14925
                </p>
            </td>
            <td valign="top" >
                <p>
                    29700
                </p>
            </td>
            <td valign="top" >
                <p>
                    59100
                </p>
            </td>
            <td valign="top">
                <p>
                    60000
                </p>
            </td>
            <td valign="top" >
                <p>
                    63326
                </p>
            </td>
            <td valign="top" >
                <p>
                    120000
                </p>
            </td>
            <td valign="top" >
                <p>
                    133226
                </p>
            </td>
            <td valign="top" >
                <p>
                    180000
                </p>
            </td>
            <td valign="top" >
                <p>
                    210382
                </p>
            </td>
            <td valign="top" >
                <p>
                    240000
                </p>
            </td>
            <td valign="top" >
                <p>
                    301895
                </p>
            </td>
            <td valign="top" >
                <p>
                    300000
                </p>
            </td>
            <td valign="top" >
                <p>
                    411117
                </p>
            </td>
            <td valign="top" >
                <p>
                    360000
                </p>
            </td>
            <td valign="top" >
                <p>
                    526725
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top" >
                <p>
                    7
                </p>
            </td>
            <td valign="top" >
                <p>
                    10000
                </p>
            </td>
            <td valign="top" >
                <p>
                    29850
                </p>
            </td>
            <td valign="top">
                <p>
                    59400
                </p>
            </td>
            <td valign="top" >
                <p>
                    118200
                </p>
            </td>
            <td valign="top" >
                <p>
                    120000
                </p>
            </td>
            <td valign="top" >
                <p>
                    126652
                </p>
            </td>
            <td valign="top" >
                <p>
                    240000
                </p>
            </td>
            <td valign="top" >
                <p>
                    266452
                </p>
            </td>
            <td valign="top" >
                <p>
                    360000
                </p>
            </td>
            <td valign="top">
                <p>
                    420764
                </p>
            </td>
            <td valign="top">
                <p>
                    480000
                </p>
            </td>
            <td valign="top">
                <p>
                    603790
                </p>
            </td>
            <td valign="top" >
                <p>
                    600000
                </p>
            </td>
            <td valign="top" >
                <p>
                    822233
                </p>
            </td>
            <td valign="top" >
                <p>
                    720000
                </p>
            </td>
            <td valign="top" >
                <p>
                    1053450
                </p>
            </td>
        </tr>
    </tbody>
</table>

</div>
         
        </p>
      </div>
    </aside>
    <section class="right-box">
  <div class="formbox">
        <h2>Login</h2>
        <form>
          <p>
            <input type="text" placeholder="Login" id="login" >
          </p>
          <p>
            <input type="text" placeholder="Password"  id="pswd" >
          </p>
            <span  style="display:none; font-size: 12px;color: #F00;margin-left: 31px;" class="invaliduser">Invalid User</span>
          <span><a href="#">Forgot Password?</a></span>
          <p>
            <input type="button" value="Login" >
          </p>
        </form>
      </div>
      <div class="important-notice">
        <h3>Important Notifications<span> </span></h3>
        <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries</p>
      </div>
    </section>
  </article>
</div>
<footer class="footer">
  <div class="footerin">
    <div class="foot1">
      <ul>
        <li><a href="#">Credit Card</a></li>
        <li><a href="#">Savings Account</a></li>
        <li><a href="#">Life Insurance</a></li>
        <li><a href="#">Salary Account</a></li>
        <li><a href="#">Debit Card</a></li>
        <li><a href="#">Current Account</a></li>
        <li><a href="#">Demat Account</a></li>
        <li><a href="#">Forex Card</a></li>
      </ul>
    </div>
  </div>
</footer>
</body>
</html>
